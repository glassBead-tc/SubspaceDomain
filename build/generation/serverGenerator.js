...
export class ServerGenerator {
    constructor(templateManager, dependencyResolver, promptAnalyzer) {
        this.templateManager = templateManager;
        this.dependencyResolver = dependencyResolver;
        this.promptAnalyzer = promptAnalyzer;
    }
    async generateServer(prompt, options = {
        format: 'typescript',
        ...
        const validationResult = await this.promptAnalyzer.validateCapabilities(analysis.capabilities);
        if (!validationResult.valid) {
            throw new Error(`Invalid capabilities: ${validationResult.errors.map(e => e.message).join(', ')}`);
        }
        // Resolve dependencies
        const dependencies = await this.dependencyResolver.analyzeDependencies(analysis.capabilities);
        ...
        const code = await this.generateServerCode(template, analysis.capabilities, serverName, options);
        // Create metadata
        const metadata = {
    ...
    async generateServerCode(template, capabilities, serverName, options) {
    ...
    async generateHandlers(capabilities) {
        const handlers = [];
    ...
    generateResourceHandler(name, resource) {
        return `
    this.server.setRequestHandler(
      ${name}ResourceSchema,
      async (request) => {
    ...
    generateToolImplementation(name, tool) {
    ...
    generateResourceImplementation(name, resource) {
    ...
    async predictResourceNeeds(capabilities) {
