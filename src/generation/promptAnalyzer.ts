...
export class PromptAnalyzer {
  ...
  async validateCapabilities(
    ...
    if (capabilities.tools) {
      for (const [name, tool] of Object.entries(capabilities.tools) as [string, MCPTool][]) {
    ...
    if (capabilities.resources) {
      for (const [name, resource] of Object.entries(capabilities.resources) as [string, MCPResource][]) {
  ...
  private async extractCapabilities(prompt: string): Promise<ServerCapabilities> {
  ...
  private analyzeAuthRequirements(prompt: string): AuthConfig {
    // Default to no auth
  ...
  private analyzeDataFlow(prompt: string): DataFlowConfig {
    const dataFlow: DataFlowConfig = {
      inputSources: [],
      outputTargets: [],
      transformations: [],
      patterns: []
   ...
   */
  public extractDependencies(prompt: string): DependencyMap {
    const requiredDependencies: DependencyMap['required'] = {};
  ...
  private isLikelyPackage(packageName: string, context: string, index: number): boolean {
  ...
  private findMatchesWithIndices(text: string, regex: RegExp): Array<{ text: string, startIndex: number, endIndex: number }> {
    const matches: Array<{ text: string, startIndex: number, endIndex: number }> = [];
    let match;
  ...
  private findFirstMatch(text: string, regex: RegExp): { text: string, startIndex: number } | null {
