...
export class AccessControlManager {
  ...
  public checkAccess(
    ...
    try {
      ...
      throw new SecurityError(
        SecurityErrorType.ACCESS_DENIED,
   ...
   * Remove access rule
   */
  public removeRule(rule: AccessRule): void {
   ...
   */
  private matchIpAddress(ip: string, patterns: string[]): boolean {
    return patterns.some(pattern => {
   ...
   */
  private validateRules(rules: AccessRule[]): void {
   ...
   */
  private validateRule(rule: AccessRule): void {
    // Validate level
    ...
    if (rule.conditions) {
      ...
      if (rule.conditions.ipRestriction) {
        if (!Array.isArray(rule.conditions.ipRestriction)) {
          throw new SecurityError(
            SecurityErrorType.INVALID_CONFIG,
            'IP restriction must be an array'
          );
        }
      ...
      if (rule.conditions.locationRestriction) {
        if (!Array.isArray(rule.conditions.locationRestriction)) {
          throw new SecurityError(
            SecurityErrorType.INVALID_CONFIG,
            'Location restriction must be an array'
          );
        }
